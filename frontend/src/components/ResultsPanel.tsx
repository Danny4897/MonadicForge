  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 text-gray-400">
        <div className="h-8 w-8 rounded-full border-4 border-leaf-500 border-t-transparent animate-spin" />
  ...
  if (!result) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-2 text-gray-400 px-6 text-center">
        ...
        <p className="text-sm">Paste your C# code and press <kbd className="px-1 py-0.5 bg-gray-100 rounded text-xs font-mono">Ctrl+Enter</kbd> to analyze.</p>
      </div>
    )
  }
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-shrink-0 flex items-center gap-4 p-4 border-b border-gray-100">
        ...
        <div className="flex flex-col text-sm text-gray-600">
          ...
          {result.fileName && <span className="text-xs text-gray-400 font-mono">{result.fileName}</span>}
          <div className="mt-1 flex gap-2 text-xs">
            {grouped.Error.length > 0 && <span className="text-red-600">{grouped.Error.length} errors</span>}
            {grouped.Warning.length > 0 && <span className="text-amber-600">{grouped.Warning.length} warnings</span>}
            {grouped.Info.length > 0 && <span className="text-blue-600">{grouped.Info.length} info</span>}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {SEVERITIES.map(sev =>
          grouped[sev].length > 0 ? (
            <div key={sev}>
              <h3 className="text-xs font-semibold uppercase tracking-wide text-gray-400 mb-2">{sev}s</h3>
